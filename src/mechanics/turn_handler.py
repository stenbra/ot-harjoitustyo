...
class TurnHandler:
    ...
    def set_computer_cards(self):
        computer_cards={}
        computer_cards[self.players[1]] = self.cardpool.get_actions_for_computer_opponent()
        ...
        for player_card_dict in self.played_cards:
            for player in player_card_dict:
                ...
                for card in player_card_dict[player]:
                    ...
                    counter += 1
                for i in range(2-counter): #the 2 is a magic number which is the cardslot max, ill get back to this at some point hopefully
                    stats = self.cardpool.card_stats["NONE"]
                    cards_with_stats.append(stats)
                players_cards[player] =cards_with_stats
        for i in range(2):
            self.card_comparer.compare_cards(players_cards[players[0]][i],players_cards[players[1]][i],players[0],players[1])
            time.sleep(2)
        self.end_turn()
